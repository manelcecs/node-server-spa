...
module.exports = {
    ...
    editPackage: () => {
        try{
            console.log(
                chalk.hex('#FFAB60')('Updating package json to add script `nsspa-deploy`')
            );

            var packageDir = __dirname + '/../package.json';
            var package = require(packageDir);
            
            var scripts = package.scripts;

            var edited = Object.defineProperty(scripts, "nsspa-deploy", {
                enumerable: true,
                writable: true,
                configurable: true,
            ...
            package.scripts = edited;

            fs.writeFileSync(packageDir, JSON.stringify(package, null, 4));
